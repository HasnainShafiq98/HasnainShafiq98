...
`03/2026 – Present`
- Built a **multi-role B2B SaaS platform** with CRM and customer self-service portals.
- Developed a **digital contract-signing workflow** with secure authentication and audit tracking.
- Created **commission-management modules** for dealers and employees.
- Deployed scalable apps on **Cloudflare Workers, D1, R2 & Durable Objects**.

<sub>`React` · `TypeScript` · `Cloudflare` · `REST APIs` · `Durable Objects`</sub>

### 🟣 Tech & Development — *Working Student* · **Nao Intelligence**, Munich
`07/2025 – 01/2026`
- Streamlined Excel data into a **SharePoint-synced Streamlit dashboard** (Graph API + Azure Auth).
- Engineered an **automated invoice-extraction script** for O2 & Telekom invoices (Comvado).
- Delivered a **procurement analytics dashboard** with clean visual reporting (Alunorf).
- Designed an **AI-driven category recommender** to classify large document batches (Savify AG).

<sub>`Python` · `PostgreSQL` · `LangChain` · `Power BI` · `Streamlit` · `Docker` · `Azure` · `Claude`</sub>

### 🟣 Generative AI & Data Analytics — *Working Student* · **Cuba Buddy**, Berlin
`08/2024 – 05/2025`
- Built an **AI tool to audit sales calls** and surface performance gaps.
- Resolved a **30% ad-to-CRM data mismatch** via server-side GTM and optimized tracking.
- Improved **data accuracy +20%** and **attribution precision +10%** by configuring GA4 through GTM.
- Automated the lead workflow and enabled enhanced conversion tracking across Google & Bing Ads.

<sub>`OpenAI` · `LLMs` · `GA4` · `GTM` · `Zapier` · `Looker Studio` · `Pipedrive`</sub>

### 🟣 Data Associate (Data Analyst) · **Marketlytics**, Karachi
`12/2022 – 01/2024`
- Restored broken analytics with GA4 and server-side tracking across multiple businesses.
- Automated **GA4 + Segment + BigQuery** integration — saving **10+ hours weekly**.
- Increased **add-to-cart rate by 8.3%** by optimizing funnel drop-offs and targeting.
- Led agile analytics audits to validate tracking and support data-driven decisions.
...
**Research Publication · IAES Prime**
...
| | |
|---|---|
| 🎓 **M.Sc. Artificial Intelligence** | Brandenburgische Technische Universität, Cottbus · `2023 – Present` |
